...
equity = pd.read_pickle('equity') #당기순이익
...
for n in range(3,66):
    ...
    data = pd.concat([data_big, size_FIF_insider[n], equity[n], equity[n+1],cash_div[n]],axis=1,join='inner',ignore_index=True)
...
#    &(data['1/div_yield']>div_min)]
    ...
    data_q=data
    market_capital=np.sum(data_q['size'])
    ...
    std_inv_pbr=np.sqrt(np.sum(np.square(data_q['1/pbr']-mu_inv_pbr)*data_q['market_weight']))
    std_inv_per=np.sqrt(np.sum(np.square(data_q['1/per']-mu_inv_per)*data_q['market_weight']))
    std_inv_div=np.sqrt(np.sum(np.square(data_q['1/div_yield']-mu_inv_div)*data_q['market_weight']))
...
#    return_data.iloc[0,n-3]=np.mean(data[11])
    return_data.iloc[0,n-3]=np.sum(data[11]*data[12])    
    if n == 65 : 
        pass
...
#    &(data['1/div_yield']>div_min)]
    ...
    market_capital=np.sum(data['size'])
    data=data.assign(market_weight2=data_q['size']/market_capital)
    np.sum(data['market_weight2'])
    
    
    
    data = pd.concat([data,rtn[n-3]],axis=1,join='inner',ignore_index=True)
    #수익률 매칭
    return_data.iloc[0,n-3]=np.mean(data[12])
#    return_data.iloc[0,n-3]=np.sum(data[12]*data[11])
    if n == 65 : 
        pass
    return_final=np.product(return_data,axis=1)
