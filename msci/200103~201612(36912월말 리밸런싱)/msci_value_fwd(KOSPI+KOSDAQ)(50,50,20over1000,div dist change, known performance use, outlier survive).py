# -*- coding: utf-8 -*-
"""
Created on Fri Jun  2 09:59:55 2017
...
#dividend yield = 0 인 애들을 뺴고 div_z를 구하고 다시 분포의 최 하값에 대입
#==============================================================================
...
for n in range(3,67):
  ...
  #배당 주는 기업과 주지 않는 기업 따로 저장
    data_div_1 = data_div[data_div['div_yield']>0]
    data_div_2 = data_div[data_div['div_yield']==0]

    # 시가총액비중 구함 
    data_pbr_cap = np.sum(data_pbr['size_FIF_wisefn'])
    data_per_cap = np.sum(data_per['size_FIF_wisefn'])
    data_div_cap = np.sum(data_div_1['size_FIF_wisefn'])
    ...
    data_div_1 = data_div_1.assign(market_weight=data_div_1['size_FIF_wisefn']/data_div_cap)
    
    # 시총가중 평균 
    mu_inv_pbr=np.sum(data_pbr['1/pbr']*data_pbr['market_weight'])
    mu_inv_per=np.sum(data_per['1/per']*data_per['market_weight'])
    mu_inv_div=np.sum(data_div_1['div_yield']*data_div_1['market_weight'])
    ...
    std_inv_div=np.sqrt(np.sum(np.square(data_div_1['div_yield']-mu_inv_div)*data_div_1['market_weight']))
    
    data1=(data_pbr['1/pbr']-mu_inv_pbr)/std_inv_pbr
    data1.name= 'pbr_z'
    data2=(data_per['1/per']-mu_inv_per)/std_inv_per
    data2.name= 'per_z'
    data3=(data_div_1['div_yield']-mu_inv_div)/std_inv_div
    data3.name= 'div_z'
    
    #배당을 가장 적게 주는 기업의 z 값을 저장
    min_data_div_1=np.min(data3)
    data_div_2 = data_div_2.assign(div_z = min_data_div_1-0.5)
    data_div_2 = data_div_2['div_z']
    
    #배당 주는 기업과 주지 않는 기업의 z 값 합침
    data3 = pd.concat([data3,data_div_2],axis=0)
