# -*- coding: utf-8 -*-
"""
Created on Tue Jun 27 15:01:22 2017
...
#PCR = price/주당영업현금흐름 = 시가총액/영업현금흐름(TTM)
#==============================================================================
...
#operating_cash_flow = pd.read_excel('msci_rawdata_kospi_25811.xlsm',sheetname='영업현금흐름1',header=None)
#operating_cash_flow.to_pickle('operating_cash_flow')
#operating_cash_flow_kq = pd.read_excel('msci_rawdata_kosdaq_25811.xlsm',sheetname='영업현금흐름1',header=None)
#operating_cash_flow_kq.to_pickle('operating_cash_flow_kq')
...
operating_cash_flow = pd.read_pickle('operating_cash_flow') #영업현금흐름(TTM)
...
operating_cash_flow_kq = pd.read_pickle('operating_cash_flow_kq') #영업현금흐름(TTM)
...
operating_cash_flow_sum = pd.concat([operating_cash_flow,operating_cash_flow_kq],axis=0,ignore_index=True)
...
#select_z_score=1 #pbr,per,div,ocf 
#select_z_score=2 #per,ocf
#select_z_score=3 #per,div,ocf
#
...
for n in range(3,68):
    ...
    data = pd.concat([data_big, size_FIF_wisefn_sum[n], equity_sum[n], ni_12m_fw_sum[n],cash_div_sum[n],size_sum[n],rtn_sum[n-3],sector_sum[n],rtn_month_sum[3*(n-3)],rtn_month_sum[3*(n-3)+1],rtn_month_sum[3*(n-3)+2],operating_cash_flow_sum[n]],axis=1,join='inner',ignore_index=True)
    data.columns = ['name','group','size_FIF_wisefn','equity','ni_12fw','cash_div','size','return','sector','return_month1','return_month2','return_month3','operating_cash_flow']
...
#        samsung['z_score'] = 0
    ...
    data['1/pcr']=data['operating_cash_flow']/data['size']
    ...
    data_pcr = data[data['1/pcr'].notnull()]
    #양 끝 5% 구함
...
#    data_div = data_div[(data_div['div_yield']>div_min)&(data_div['div_yield']<div_max)]
    ...
    data_pcr_cap = np.sum(data_pcr['size_FIF_wisefn'])
    ...
    data_pcr = data_pcr.assign(market_weight=data_pcr['size_FIF_wisefn']/data_pcr_cap)
    ...
    mu_inv_pcr=np.sum(data_pcr['1/pcr']*data_pcr['market_weight'])
    ...
    std_inv_pcr=np.sqrt(np.sum(np.square(data_pcr['1/pcr']-mu_inv_pcr)*data_pcr['market_weight']))
    ...
    data3.name= 'div_z'
    data4=(data_div['1/pcr']-mu_inv_pcr)/std_inv_pcr
    data4.name= 'pcr_z'
    
    if select_z_score==1:      #pbr,per,div,ocf 
        result = pd.concat([data, data1, data2, data3, data4], axis = 1)
        
        # np.nanmean : nan 값 포함해서 평균 내기!!
        result = result.assign(z_score=np.nanmean(result.iloc[:,[17,18,19,20]],axis=1))
    elif select_z_score==2:  #per,ocf
        result = pd.concat([data, data2, data4], axis = 1)
        
        # np.nanmean : nan 값 포함해서 평균 내기!!
        result = result.assign(z_score=np.nanmean(result.iloc[:,[17,18]],axis=1))
    elif select_z_score==3:  #per,div,ocf
        result = pd.concat([data, data2, data3, data4], axis = 1)
        
        # np.nanmean : nan 값 포함해서 평균 내기!!
        result = result.assign(z_score=np.nanmean(result.iloc[:,[17,18,19]],axis=1))
